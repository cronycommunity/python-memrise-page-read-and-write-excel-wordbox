...
browser.get('https://www.memrise.com/course/5542612/c1-wortschatz-mit-satze/')
...
for i in tqdm.tqdm(levelist1):
    browser.get('https://www.memrise.com/course/5542612/c1-wortschatz-mit-satze/{}/'.format(i))    
    all_spans = browser.find_elements_by_xpath("//*[contains(.,'text')]")                          
    ...
    list1 = [x.strip().split("\n") for x in list1]
    list1=list1[0]
    list1=list1[list1.index('Bereit zum lernen       Bereit zum Wiederholen')+1:list1.index('Über uns')]      
    df=pd.DataFrame({'almanca':list1[0::2],'turkce':list1[1::2]})
    if not os.path.exists('MemriseTable.csv'):
        df.to_csv('MemriseTable.csv')
    else:
        df.to_csv('MemriseTable.csv',mode='a',header=False)
